...
void inserimento()              //Questa void corrisponde all'inserimento di un disco. A seconda della categoria scelta dall'utente, verrà inserito, se possibile, un disco.
{
    int categoria;
        ...
        case 1: if(i<3)
                {
                    ...
                    printf("Errore! Spazio esaurito! Non si possono piu' inserire dischi Jazz\n");
            break;
        case 2: if(j<3)
                {
                    ...
                    printf("Errore! Spazio esaurito! Non si possono piu' inserire dischi Rock\n");
            break;
        case 3: if(k<3)
                {
                    ...
                    printf("Errore! Spazio esaurito! Non si possono piu' inserire dischi Punk\n");
            break;
    }
}

void elimina_disco()                //Questa void corrisponde all'eliminazione di un disco. Una volta inseriti dei dischi, l'utente può scegliere la seconda opzione, cioè proprio quella di eliminare un disco inserito.
{                                   //L'utente inserisce il titolo da ricercare e se questo disco viene trovato, viene eliminato.
    int a;
    char tit[50];
    fflush(stdin);
...
int minimo(int x, int y)                //Questa function serve per trovare il minimo tra i due valori che passiamo nelle successive function
{
    if(x<y)
        return x;
    else
...
int min_arrayjazz(Archivio JAZZ[], int n)           //Questa function e le successive servono per trovare il disco con il prezzo minimo. Ogni categoria ha la propria function
{
    if(n==1)
...
int disco_p_minore()            //Questa function corrisponde alla ricerca del disco con il prezzo minore. L'utente inserisce la categoria dopve si vuole cercare il prezzo minore e successivamente viene stampato.
{
    int x, i;
    float minimo;
        ...
        case 1: minimo=min_arrayjazz(JAZZ,3);
                for(i=0;i<3;i++)
                {
                    if(JAZZ[i].prezzo==minimo)
                        printf("Il disco con il prezzo minimo %f e' %s \n",JAZZ[i].prezzo, JAZZ[i].titolo);
                }
            break;
